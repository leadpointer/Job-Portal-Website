...
try {
	...
	$current_year = date('Y');
	$current_year = intval($current_year); // Convert $current_year to an integer
	$byear = intval($byear);               // Convert $byear to an integer
	$myage = $current_year - $byear;       // Now subtraction will work correctly
	$myedu = $row['education'];
	$mytitle = $row['title'];
	$mymail = $row['email'];
	}
...
<head>
	...
	<title>TMS Jobs - <?php echo "$myfname"; ?> <?php echo "$mylname"; ?></title>
